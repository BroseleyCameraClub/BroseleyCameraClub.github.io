...
{% for image in site.static_files %}
    {% if image.path contains 'assets/img/November24_Lancaster/' %}
<div class="Number">{{ image.basename }}</div>

<!-- This runs and checks if there is a matching author in the file -->
    {% for nov24LanAuth in site.data.nov24LanAuth   %}
      {% if image.basename == nov24LanAuth.Img_Name %}
<div class="subName">By - {{ AAAnov24LanAuth.Author }}</div>
      {% endif %}
    {% endfor %}
...
<div class="grid__item" data-size="1280x1280">  
    ...
    {% for nov24LanAuth in site.data.nov24LanAuth %}
        {% if image.basename == nov24LanAuth.Img_Name %}
<div class="description description--grid">{{ image.basename }} - {{ AAAnov24LanAuth.Author }}</div>
        {% endif %}
    {% endfor %}
