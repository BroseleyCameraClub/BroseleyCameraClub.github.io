...
{% for image in site.static_files %}
    {% if image.path contains 'assets/img/July24_Monthly/' %}
<div class="Number">{{ image.basename }}</div>

<!-- This runs and checks if there is a matching author in the file -->
    {% for july24MonAuth in site.data.july24MonAuth   %}
      {% if image.basename == july24MonAuth.Img_Name %}
<div class="subName">By - {{ AAAjuly24MonAuth.Author }}</div>
      {% endif %}
    {% endfor %}
...
<div class="grid__item" data-size="1280x1280">  
    ...
    {% for july24MonAuth in site.data.july24MonAuth %}
        {% if image.basename == july24MonAuth.Img_Name %}
<div class="description description--grid">{{ image.basename }} - {{ AAAjuly24MonAuth.Author }}</div>
        {% endif %}
    {% endfor %}
